export default class Cpf {
  ...
  private isValidLength = function (cpf: string): boolean {
    return cpf.length === 11;
  };

  private hasAllDigitsEqual = function (cpf: string): boolean {
    const [firstDigit] = cpf;
    return [...cpf].every(digit => digit === firstDigit);
  };

  private calculateCheckDigit = function (cpf: string, factor: number): number {
    let total = 0;
  ...
  private extractCheckDigit = function (cpf: string): string {
    return cpf.slice(-2);
  };
}
